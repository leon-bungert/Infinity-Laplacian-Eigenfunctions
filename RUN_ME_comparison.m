% This code numerically computes the first eigenfunction and potential of the
...
max_broyden     = 0;
broyden         = 'bad';
samples         = 1*120;       % has to be even!!!!
stencil_shape   = 'full';   % valid stencils: full, square
neighborhood_size = 11;     % at least 3
...
if save2disk
    foldername = ['comparison', '_', shape, '_', 'high_ridge_', num2str(high_ridge),...
...
for run = [1,2]
    ...
    while max(rel_change,scheme_accuracy) > TOL && iteration <= max_iterations    
        ...
        if save2disk && mod(iteration, savingFreq) == 0
            filename = ['solution_', problem, '_', 'grid_size_', strrep(num2str(step_size),'.','-'),... 
        ...
        if visualize && mod(iteration, visFreq) == 0
            fig1;
            subplot(1,2,1);
            surf(x,y,solution.*mask_vis); title([problem,' at iteration ',num2str(iteration)]); 
            axis equal;
            view(-30,20); drawnow;
            subplot(1,2,2);
    ...
    if run == 1
        inf_potential = solution;
    end
    if run == 2
        inf_ground_state = solution;
    end
    toc 
    %%
    if save2disk
...
difference = inf_ground_state-inf_potential;

fig3 = figure(3);
fig3.PaperOrientation = 'landscape';
surf(x,y,difference.*mask_vis);  
view(-30,20); drawnow;
if save2disk
    print([outputfolder,'/','difference_surf_',filename],'-dpdf','-r300','-fillpage')
end
